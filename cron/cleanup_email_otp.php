...
/**
 * CLI / cron entrypoint: delete expired or consumed email OTP codes.
...
try {
    ...
    $deleted = (new EmailOtpCleanup($pdo))->run();
    $message = 'cleanup_email_otp deleted=' . $deleted;
    if ($isCli) {
    ...
    $err = 'cleanup_email_otp failed: ' . $e->getMessage();
    if ($isCli) {
